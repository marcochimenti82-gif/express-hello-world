...
function isOperatorRequest(speech, session) {
  const tt = normalizeText(speech);
  if (!tt) return false;

  // "Trigger forti": richieste esplicite di assistenza umana
  const strong =
    tt.includes("operatore") ||
    tt.includes("assistenza") ||
    tt.includes("aiuto") ||
    tt.includes("parlare con qualcuno") ||
    tt.includes("parlare con una persona") ||
    (tt.includes("parlare con") && tt.includes("persona")) ||
    (tt.includes("mi passi") && (tt.includes("operatore") || tt.includes("persona"))) ||
    (tt.includes("mi metti in contatto") && (tt.includes("operatore") || tt.includes("persona")));

  // "Trigger debole": la parola "persona" da sola (può generare falsi positivi tipo "una persona celiaca")
  const weakPersona = tt.includes("persona");

  // STEP 3: se il testo parla chiaramente di intolleranze/allergie, non considerare "persona" come richiesta operatore
  const allergyCtx =
    tt.includes("celiac") ||
    tt.includes("celia") ||
    tt.includes("glutin") ||
    tt.includes("senza glut") ||
    tt.includes("intoller") ||
    tt.includes("allerg") ||
    tt.includes("lattos");

  if (allergyCtx) {
    // in contesto allergie/intolleranze accettiamo solo trigger forti espliciti
    return strong;
  }

  // STEP 2: nello step "richieste particolari/intolleranze" (prenotazione tavolo) ignora i trigger deboli
  const step = session?.step;
  const operatorState = session?.operatorState || "";
  const inNotesStep = step === 5 || operatorState === "manage_modify_set_notes";

  if (inNotesStep) {
    return strong;
  }

  // comportamento generale (compatibile con la versione precedente)
  return strong || weakPersona;
}
function pad2(n) {
...
async function handleVoiceRequest(req, res) {
  ...
  try {
    ...
    if (!emptySpeech && isOperatorRequest(speech, session)) {
      session.operatorState = "await_reason";
