...
function getCredentials() {
  ...
  const unquoted = rawEnv.replace(/^"(.*)"$/, '$1');

  if (unquoted.startsWith('{')) {
    return JSON.parse(unquoted);
  }

  const base64Payload = unquoted.replace(/\s+/g, '');
  ...
  try {
    ...
    throw new Error(
      'Invalid GOOGLE_SERVICE_ACCOUNT_JSON_B64: provide a single-line base64 value or raw JSON.'
    );
  }
}
